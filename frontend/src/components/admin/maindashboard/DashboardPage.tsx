...
const DashboardPage: React.FC = () => {
  const [activeAddCategoryPopup, setActiveAddCategoryPopup] =
    useState<boolean>(false);
  const [sideMenuClickedItemsDisplay, setSideMenuClickedItemsDisplay] =
    useState<number>(0);
  const [sideClickCategory, setSideClickCategory] =
    useState<string>("mainSideBarData");

  const dashboardData: DashBoardSideBarData =
    dashBoardSideBarData.dashBoardSideBarData;

  const topNavRight = ["jin", "hungu"];
  ...
  const slideBarClickHandler = (i: number, category: string) => {
    setSideMenuClickedItemsDisplay(i);
    setSideClickCategory(category);
  };
  return (
    <>
      <div className="min-w-[1024px] flex content-center sm:max-w-screen-xl mx-auto">
        <div className="one w-[240px] bg-slate-100   h-screen py-3 fixed top-0 left-0">
          ...
          <MainSide
            ...
            slideBarClickHandler={slideBarClickHandler}
          />
        </div>
        <section className="box-border relative w-full ml-[240px] h-screen overflow-y-scroll">
          <nav className=" bg-slate-100 shadow-sm z-[11] sticky top-0 box-border inline-flex flex-grow w-full  py-6 pr-6 overflow-hidden">
            <div className="top-nav-left flex flex-grow items-start">
              <div className="inline-flex self-center">
                <input
                  className="min-w-[250px] ml-2 px-4 h-8 content-center shadow rounded-l"
                  type="text"
          ...
          {sideClickCategory === "mainSideBarData" &&
            sideMenuClickedItemsDisplay === 0 && <DashboardHome />}
          {sideClickCategory === "mainSideBarData" &&
            sideMenuClickedItemsDisplay === 4 && (
              <MainCategory
      ...
      {activeAddCategoryPopup && (
        <AddNewCategory setActiveAddCategoryPopup={setActiveAddCategoryPopup} />
      )}
    </>
  );
};
